...
DB_PASSWORD="{{DB_PASSWORD}}" # <-- CHANGE THIS to a secure password
...
if [ -f "/tmp/.env.option-rocket.bak" ]; then
    ...
    REVERB_APP_ID_VAL=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 8)
    REVERB_APP_KEY_VAL=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 16)
    REVERB_APP_SECRET_VAL=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 32)
    sed -i "s/^REVERB_APP_ID=.*/REVERB_APP_ID=${REVERB_APP_ID_VAL}/" .env
    sed -i "s/^REVERB_APP_KEY=.*/REVERB_APP_KEY=${REVERB_APP_KEY_VAL}/" .env
    sed -i "s/^REVERB_APP_SECRET=.*/REVERB_APP_SECRET=${REVERB_APP_SECRET_VAL}/" .env
