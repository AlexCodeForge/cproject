<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="{}" class="{{ request()->cookie('theme', 'light') === 'dark' ? 'dark' : '' }}">
<head>
    <meta charset="UTF-8">
...
<body class="bg-stone-100 dark:bg-gray-900 text-slate-800 dark:text-gray-100 transition-colors duration-300">
    ...
    <div class="flex app-container h-screen">
        ...
        <aside id="sidebar" class="hidden lg:flex lg:flex-col h-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-r border-stone-200 dark:border-gray-700 py-4 transition-all duration-300 ease-in-out z-20">
            ...
            <div class="flex flex-col flex-grow overflow-y-auto scrollbar-hide">
                ...
                <nav id="main-nav" class="flex flex-col space-y-2 px-4 pb-4">
                    ...
                    <a href="{{ route('courses') }}" class="nav-item flex items-center p-3 rounded-xl overflow-hidden text-slate-500 dark:text-gray-400 hover:bg-stone-200 dark:hover:bg-gray-700 hover:text-slate-800 dark:hover:text-gray-200 transition-all">
        ...
        <div class="flex-1 main-content-wrapper relative">
            ...
            <div class="absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-r from-amber-200/20 to-orange-200/20 dark:from-amber-900/20 dark:to-orange-900/20 rounded-full blur-3xl -z-10"></div>
            <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-gradient-to-r from-rose-200/20 to-pink-200/20 dark:from-rose-900/20 dark:to-pink-900/20 rounded-full blur-3xl -z-10"></div>
            <main class="main-content-scrollable isolate">
                {{ $slot }}
            </main>
